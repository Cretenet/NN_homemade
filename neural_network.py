...
class NeuralNetwork:
    ...
    def add_hidden_layer(self, name, nb_neurons=1, activation_function='RELU'):
        if not self.output_layer_exists:
            ...
            self.initialize_weights(self.activation_functions[-1], self.nbLayers, self.layers_size[self.nbLayers-1], nb_neurons)
        else :
    ...
    def add_output_layer(self, name='output_layer', nb_outputs=1, activation_function='sigmoid'):
        ...
        self.initialize_weights(self.activation_functions[-1], self.nbLayers, self.layers_size[self.nbLayers-1], nb_outputs)
    ...
    def initialize_weights(self, activation_function, index, nb_first_layer, nb_second_layer):
        if activation_function == 'RELU': 
            ...
            m=1
        self.weights[index] = np.random.normal(0, m/self.layers_size[0], (nb_second_layer,nb_first_layer))
    ...
    def forward_propagation(self, input):
        ...
        for i in range(1, self.nbLayers+1) :
            ...
            b = self.biases[i]
            z = W @ first_layer + b
            second_layer = af.activation_function(z, self.activation_functions[i])
            # The following two lines are useful for backward propagation
            self.activation[i] = second_layer
            self.derivative_activation[i] = af.derivative_activation_function(z, self.activation_functions[i])
    ...
    def backward_propagation(self, truth):
        ...
        self.dl_dw[self.nbLayers-2] = np.zeros((right_layer_size,left_layer_size))
        self.dl_dw[self.nbLayers-2][0][0] = 2 * (self.activation[self.nbLayers-1][0]-truth[0]) * self.derivative_activation[self.nbLayers-2][0]*self.activation[self.nbLayers-2][0]
        self.dl_dw[self.nbLayers-2][1][0] = 2 * (self.activation[self.nbLayers-1][1]-truth[1]) * self.derivative_activation[self.nbLayers-2][1]*self.activation[self.nbLayers-2][0]
        ...
        for l in range(0,self.nbLayers-1): # Loop over the layers
            ...
            left_layer_size = self.layers_size[index_left_layer]
            right_layer_size = self.layers_size[index_right_layer]
            self.dl_dw[self.nbLayers-2-l] = np.zeros((right_layer_size,left_layer_size))
            self.dl_db[self.nbLayers-2-l] = np.zeros((right_layer_size,1))
            for j in range(0,right_layer_size):
                self.dl_db[self.nbLayers-2-l][j][0]=previous_dl_da[j] * self.derivative_activation[self.nbLayers-2-l][j]
                for k in range(0,left_layer_size):
                    self.dl_dw[self.nbLayers-2-l][j][k] = previous_dl_da[j] * self.derivative_activation[self.nbLayers-2-l][j]*self.activation[self.nbLayers-2-l][k]
            dl_da=np.zeros((left_layer_size,1))
            for k in range(0,left_layer_size):
                for j in range(0,right_layer_size):
                    dl_da[k][0] += self.weights[self.nbLayers-2-l][j][k]*self.derivative_activation[self.nbLayers-2-l][j]*previous_dl_da[j]
